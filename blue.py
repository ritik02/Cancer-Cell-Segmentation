...
for i in range(0,high):
    ...
    cv.imwrite('output_intermedite_20\\blue.tif',hsv2)
    gray = cv.cvtColor(res,cv.COLOR_BGR2GRAY)
    #cv.imwrite('output_intermedite_20\\gray.tif',gray)
    #opening1 = gray
    gray=cv.GaussianBlur(gray,(5,5),0)
    opening1=cv.morphologyEx(gray, cv.MORPH_OPEN, kernel1,iterations=2)
    #cv.imwrite('output_intermedite_20\\opening.tif',opening1)
    opening11 = cv.morphologyEx(opening1, cv.MORPH_CLOSE, kernel11,iterations=2)
    ...
    cv.imwrite('outputblue\\'+name1+'_'+str(cell)+'.tif',sure_fg)
    index1.append(i)
    count1.append(cell)
    #cv.waitKey(0)
...
for i in range(0,high):
    plt.bar(i,countgreen[i]/(1024*1024))
plt.show()
for i in range(0,high):
    plt.bar(i,count1[i])
plt.show()
